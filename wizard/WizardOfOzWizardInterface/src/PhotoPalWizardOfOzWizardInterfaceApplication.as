package {
	...
	public class PhotoPalWizardOfOzWizardInterfaceApplication extends Application {
		// Red5
		...
		public function PhotoPalWizardOfOzWizardInterfaceApplication() {
			super();
			
			trace("PhotoPalWizardOfOzWizardInterfaceApplication constructed");
		...
		private function onApplicationComplete(event:FlexEvent):void {
			...
			red5SharedObjectPastUtterancesName = "PhotoPalWizardOfOz-pastUtterances" + "_" + root.loaderInfo.parameters.session_name;
		...
		public function startAudioStream():void {
			...
			trace("PhotoPalWizardOfOzWizardInterfaceApplication.startAudioStream recording name : " + recordingName);
			
			red5SharedObject.data.recordStreamName = recordingName;
			red5SharedObject.setDirty("recordStreamName");
			trace("PhotoPalWizardOfOzWizardInterfaceApplication.startAudioStream " + red5SharedObject.data.recordStreamName);
		...
		public function startListeningToAudio():void {
			// Play the stream
			trace("PhotoPalWizardOfOzWizardInterfaceApplication.Playing audio");
		...
		private function red5OnSync(event:SyncEvent):void {
			...
			for(var i:int = 0; i < event.changeList.length; i++) {
				trace("PhotoPalWizardOfOzWizardInterfaceApplication Red5 sharedobject changed[" + i + "] " + event.changeList[i].name + ", code = " + event.changeList[i].code + ", oldValue = " + event.changeList[i].oldValue);
				
				// Check for start recording
				if(event.changeList[i].name == "recordStream") {
					trace("PhotoPalWizardOfOzWizardInterfaceApplication recordStream changed. Is now: " + red5SharedObject.data.recordStream);
				...
				if(event.changeList[i].name == "cameraOptions") {
					trace("PhotoPalWizardOfOzWizardInterfaceApplication cameraOptions changed: " + red5SharedObject.data.cameraOptions);
					if(red5SharedObject.data.cameraOptions.length == 0) {
						trace("PhotoPalWizardOfOzWizardInterfaceApplication No web cam options!")
		...
		private function joinRed5():void {
			...
			Application.application.red5NetConnection.connect("rtmp://companions.napier.ac.uk/PhotoPalWizardOfOzRed5");
