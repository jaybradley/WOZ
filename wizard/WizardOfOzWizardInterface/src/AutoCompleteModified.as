		    ...
		    if(typedTextChanged) {
			    ...
			    //typed by the user losts
  			    if(lookAhead && showDropdown && typedText!="" && !removeHighlight) {
				    ...
				    textInput.text = _typedText;
				    textInput.setSelection(cursorPosition, cursorPosition); // cursor at end of the typed text
				    removeHighlight = false;
				}
		    	...
		    	//arrows keys.
				textInput.setSelection(_typedText.length,textInput.text.length); // highlight the non-typed part
	  		} else {
	  			// When pressing key down without having typed anything
	  			// or when having typed something then clicked on an entry using the mouse and then pressed return
	  			// the text box doesn't clear or send the message because of a deeper adobe bug.
	  		}
		}
